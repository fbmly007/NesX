...
int main(int argc, char **argv)
{
	...
	if (argc > 1)
	{
		for (i = 1; i < argc; i++)
		{
			...
			if (argv[i][0] == '-')
			{
				...
				// PCE相关功能
				else if(machine->type == MACHINE_PCE)
				{
					/* cd-rom */
					if (!strcmp(argv[i], "-cd")) {
						cd_opt = STANDARD_CD;
	...
	addinst(base_inst);    // 指令
	addinst(base_pseudo);  // 伪指令

	/* add machine specific instructions and pseudos */
	addinst(machine->inst);  // 特殊指令
	addinst(machine->pseudo_inst); // 特殊伪指令
